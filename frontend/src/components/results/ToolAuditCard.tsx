...
function getBorderColor(tool: ToolAuditResult): string {
  ...
  if (tool.recommended_action === 'keep') return 'border-zinc-800';
...
function getAccentColor(tool: ToolAuditResult): string {
  if (tool.monthly_savings > 0) return 'text-emerald-400';
...
export function ToolAuditCard({ tool, index }: { tool: ToolAuditResult; index?: number }) {
  return (
    <motion.div
      ...
      className={`group relative bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden transition-all duration-300 hover:border-zinc-700 ${getGlowColor(tool)}`}
    >
      ...
      <div className="p-6 pl-8 pb-5">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
          <div className="flex-1">
            <h3 className="text-xl font-bold text-white capitalize flex flex-wrap items-center gap-3">
              {tool.tool.replace(/_/g, ' ')}
              <RecommendationBadge action={tool.recommended_action} />
            </h3>
            <p className="text-zinc-400 text-sm mt-2 flex items-center gap-2 font-medium">
              <span>{tool.seats} {tool.seats === 1 ? 'seat' : 'seats'}</span>
              <span className="w-1 h-1 rounded-full bg-zinc-700"></span>
              <span className="capitalize">{tool.current_plan} plan</span>
              <span className="w-1 h-1 rounded-full bg-zinc-700"></span>
              <span className="text-zinc-300">{formatCurrency(tool.current_spend)}/mo</span>
            </p>
          </div>
            ...
            {tool.monthly_savings > 0 ? (
              ...
              >
                <div className="flex items-center gap-1.5 justify-end mb-1">
                  ...
                  <span className="text-2xl font-bold text-emerald-400 tabular-nums drop-shadow-[0_0_10px_rgba(16,185,129,0.3)]">
                    {formatCurrency(tool.monthly_savings, { decimals: 0, showSign: true })}
                  </span>
                </div>
                <div className="text-xs font-medium uppercase tracking-wider text-emerald-500/70">monthly savings</div>
              </motion.div>
            ) : (
              <div className="flex items-center gap-1.5 bg-violet-500/10 border border-violet-500/20 rounded-full px-3 py-1.5">
                <CheckCircle2 size={14} className="text-violet-400" />
                <span className="text-xs font-semibold text-violet-400 uppercase tracking-widest">Optimal</span>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Reason section */}
      <div className="bg-zinc-900/50 border-t border-zinc-800/50 px-6 pl-8 py-5">
        <div className="flex items-start gap-3">
          <ArrowRight size={18} className={`mt-0.5 flex-shrink-0 ${getAccentColor(tool)}`} />
          <div className="min-w-0">
            <p className="text-sm text-zinc-300 leading-relaxed font-medium">{tool.reason}</p>
            {(tool.recommended_tool || tool.recommended_plan) && (
              <div className="mt-3 inline-flex items-center gap-2 rounded-lg bg-black px-3 py-1.5 border border-zinc-800">
