...
export function SavingsHero({ monthlySavings, annualSavings }: SavingsHeroProps) {
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-3xl border border-zinc-800/50 bg-black p-8 sm:p-12 shadow-2xl"
    >
      {isSaving ? (
        <>
          ...
          <div className="relative text-center z-10">
            ...
            <div className="mb-6 relative">
              <motion.span
                ...
                className="inline-block text-6xl sm:text-7xl md:text-8xl font-extrabold tracking-tight text-white tabular-nums drop-shadow-[0_0_40px_rgba(16,185,129,0.2)]"
              >
                {formatCurrency(animatedAnnual, { decimals: 0 })}
              </motion.span>
              <span className="text-xl sm:text-2xl font-medium text-emerald-500/60 ml-2">/yr</span>
            </div>
            ...
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-zinc-900/50 border border-zinc-800 backdrop-blur-sm"
            >
              <span className="text-zinc-400">That&apos;s</span>
              <span className="text-emerald-400 font-bold tabular-nums text-xl">
                {formatCurrency(animatedMonthly, { decimals: 0 })}
              </span>
              <span className="text-zinc-400">per month in immediate savings</span>
            </motion.div>
          </div>
        </>
      ) : (
        <>
          ...
          <div className="relative text-center z-10 py-10">
            ...
            <p className="text-lg sm:text-xl text-zinc-400 flex items-center justify-center gap-2 max-w-lg mx-auto">
